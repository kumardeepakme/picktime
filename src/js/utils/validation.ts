...
export const validatePickerOptions = (options: PickerOptions): boolean => {
  ...
  if (
    ...
    if (options.time?.meridiem) {
      if (
        ...
        throw new Error('"time.meridiem" value must be am/pm');
    }
  }
  ...
  // upDownKeys & wheelSpin
  if (options.upDownKeys === false && options.wheelSpin === false)
    throw new Error('atleast one of "upDownKeys" OR "wheelSpin" must be true');
